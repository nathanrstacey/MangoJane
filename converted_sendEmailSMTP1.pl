...
#	$htmlString = ' -o message-content-type=html';
#}
#while (length($theText) > 0 ) {

	$toSystem = "sendEmail -f $from -t $to -o message-file=/var/mangojane/message.txt -s $global_email_url$portToAddToString -xu $from -xp $global_email_Mail_User_Password -v -l var/mangojane/sendEmailLog.log -o tls=no -o message-content-type=html -o message-charset=utf-8 $attachmentString";
